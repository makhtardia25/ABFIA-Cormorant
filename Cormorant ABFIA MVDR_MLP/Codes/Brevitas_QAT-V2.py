...
output_x = torch.from_numpy(output_signals).float() * 0.1
...
# --- ARCHITECTURE DU RÉSEAU MVDR W16A16 ---
        class MVDR_FFNN(nn.Module):
    def __init__(self):
        super(MVDR_FFNN, self).__init__()
        
        # W16A16: Entrée du réseau quantifiée en 16-bit signés
        self.quant_input = qnn.QuantIdentity(act_quant=Int16Act, return_quant_tensor=True)

        # Toutes les couches reçoivent du A16 (input_quant) et possèdent des poids W16 (weight_quant)
        self.layer1 = qnn.QuantLinear(3, 256, bias=False, weight_quant=Int16Weight, input_quant=Int16Act, narrow_range=True)
        self.relu1  = qnn.QuantReLU(bit_width=16, act_quant=Int16Act)
        
        self.layer2 = qnn.QuantLinear(256, 512, bias=False, weight_quant=Int16Weight, input_quant=Int16Act, narrow_range=True)
        self.relu2  = qnn.QuantReLU(bit_width=16, act_quant=Int16Act)
        
        self.layer3 = qnn.QuantLinear(512, 512, bias=False, weight_quant=Int16Weight, input_quant=Int16Act, narrow_range=True)
        self.relu3  = qnn.QuantReLU(bit_width=16, act_quant=Int16Act)
        
        self.layer4 = qnn.QuantLinear(512, 1024, bias=False, weight_quant=Int16Weight, input_quant=Int16Act, narrow_range=True)
        self.relu4  = qnn.QuantReLU(bit_width=16, act_quant=Int16Act)
        
        # Dernière couche : Entrée 16-bit, Poids 16-bit, Sortie 16-bit
        self.layer5 = qnn.QuantLinear(1024, 32, bias=False, weight_quant=Int16Weight, input_quant=Int16Act, narrow_range=True)
        self.quant_output = qnn.QuantIdentity(act_quant=Int16Act, return_quant_tensor=True)
        
    def forward(self, x):
        # 1. Quantification de l'entrée (Angles)
        x = self.quant_input(x)
        
        # 2. Couches cachées associant les multiplicateurs matériels 16x16 bits
        x = self.relu1(self.layer1(x))
        x = self.relu2(self.layer2(x))
        x = self.relu3(self.layer3(x))
        x = self.relu4(self.layer4(x))
        
        # 3. Génération des 32 composantes des poids MVDR en précision 16-bit
